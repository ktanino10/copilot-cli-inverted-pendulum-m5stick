...
今回の挙動は、単純な「PIDの値が合っていない」だけではなく、**倒立振子がもともと不安定な系であること**、**センサの遅れ**、**D項の位相**、**FS90Rサーボの不感帯・個体差**が重なって起きていると考えると整理しやすい。

#### 1. 倒立振子は「倒れる方向へ走って支点を入れる」制御

倒立振子は、普通の振子と逆で、直立が自然に安定しない。少し前に傾くと重心はさらに前へ倒れようとする。そこで車輪を前へ動かし、**車輪という支点を重心の下へ入れる**ことで倒れないようにする。
...
つまり「傾いた向きと同じ方向へ走る」こと自体は正しい。ただし、走る量が少なすぎると倒れ、走りすぎると反対側へ倒れる。今回の「前にも後ろにも倒れる」「ある傾きから走りながら倒れる」は、**補正方向は概ね合っているが、補正量・タイミング・モーター特性がまだ合っていない**状態と読める。

#### 2. P項は起き上がる力、強すぎると振動になる
...
- `kp` が小さい: 傾いても力が足りず、そのまま倒れる
- `kp` が大きい: すばやく戻ろうとするが、戻りすぎて反対側へ倒れる
- `kp` が大きすぎる: 前後に行き過ぎる動きが続き、小刻みな振動になる

今回「時間は伸びたが前後どちらにも倒れる」「振動しながらある傾きから走る」という観察は、P項だけでは止まりきらず、**D項のブレーキやモーターの効き方とのバランスが必要**な状態を示している。

#### 3. D項はブレーキ。ただしノイズや遅れがあると振動源になる

D項は「傾きの速度」を見てブレーキをかける項。倒立振子ではとても重要で、P項だけだと行き過ぎやすい。
...
理想的には、D項は「倒れ始めた瞬間に先回りして止める」働きをする。しかし実機では次の問題がある。

| 問題 | 起きること |
|------|------------|
| ジャイロにノイズがある | D項が細かく揺れ、モーター出力も細かく反転する |
| 角度 `Angle` とジャイロ `dAngle` のスケールが合わない | ブレーキが強すぎたり弱すぎたりする |
| フィルタで角度が遅れる | 実際には倒れ始めているのに、制御が少し遅れて反応する |
| D項が大きすぎる | 小さい揺れに過剰反応し、出力飽和と激しい振動になる |

`failure_log_oscillation.csv` はこの典型で、D項や出力が強く、`power` が大きく振れてから `ANGLE_LIMIT` を超えている。これは「力不足」ではなく、**閉ループのゲインが高すぎる、またはD項がノイズ/位相遅れに反応しすぎている**と読む。

#### 4. I項・速度項は「走り続ける問題」を止めるが、早く入れると暴れる

角度のP+D制御だけでも一瞬は立つが、車輪位置や速度を見ていないため、ロボット全体がじわじわ移動し続けることがある。これを止めるために I項・速度項・位置項を使う。

ただし、今回のログでは `ki=0` にしていても、`kspd` や `kdst` が残ると走り込みが増幅した。これは、角度制御がまだ安定していない段階で速度/位置の補正を足すと、**姿勢を戻す力**と**場所へ戻す力**が競合して、かえって振動や暴走を作るため。

| 項 | 役割 | 今回の扱い |
|----|------|------------|
| P | 傾いた分だけ戻す主力 | 必要。ただし強すぎると振動 |
| D | 倒れる速度を止めるブレーキ | 必要。ただしノイズ・位相に注意 |
| I | 小さなずれの蓄積を消す | 現時点ではOFFが安全 |
| kspd/kdst | 走り続け・位置ずれを抑える | 姿勢P+Dが安定してから戻す |

そのため現在は、まず `ki=0`, `kspd=0`, `kdst=0` の **P+Dだけの基準状態**を作り、転倒ログで原因を見てから戻す方針にしている。

#### 5. 今の角度 `acc[2]` は「物理角度」ではなく「制御用の偏差」として読む

現在のコードでは、前後の符号を安定して分けるために次の式を使っている。

```cpp
getPitch() = (acc[2] - 1.0) * 57.3
```

これは厳密な物理角度ではなく、実機のIMU取り付け向き・直立姿勢・重力成分を含んだ **制御用の傾きエラー**として扱っている。大事なのは、まず小さい角度範囲で、

1. 前に傾けたときと後ろに傾けたときで符号が分かれる
2. 傾きが大きくなるほど値も大きくなる
3. その値に対してモーターが正しい方向へ動く

の3つが成り立つこと。

ただし、角度は `acc[2]` 由来、D項は `gyro[0]` 由来なので、両者の位相やスケールが完全に一致しているとは限らない。これが次回の「`gyro[0]` のD項」と「`Angle` 差分から作るD項」を比較する理由。

#### 6. 後ろへ逃げる挙動は、符号逆ではなく「片方向の効き不足」と読む

`failure_log_backward_run.csv` では、Angle が小さい範囲にいる一方で、`power` は負側に偏り続けていた。これは、制御器が「後ろ側へ補正しろ」と命令しているのに、実機が十分に動いていない可能性を示す。

考えられる原因は以下。

| 原因 | 説明 |
|------|------|
| FS90Rの不感帯 | 小さいPWM差ではサーボが動かない、または片輪だけ動く |
| 前進/後退の非対称 | 同じ絶対値の `power` でも、前進側と後退側で実際の速度が違う |
| 左右差 | 左右サーボのニュートラル点や摩擦が違い、直進補正にならない |
| 出力を段差で足した副作用 | `minp=80` のような最低出力は不感帯を越えるが、段差的に効いて振動を作る |

そのため `pneg` を上げると立つ時間が伸びた一方、上げすぎると小刻み振動が増えた。理論的には、これは**片方向のゲイン不足を補うと安定余裕が減り、振動しやすくなる**という典型的なトレードオフ。
